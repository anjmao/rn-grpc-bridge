...
import * as EnumFormatter from './EnumFormatter';
import * as ExtensionFormatter from './ExtensionFormatter';
import * as OneofFormatter from './OneofFormatter';
...
function hasFieldPresence(field: FieldDescriptorProto, descriptor: FileDescriptorProto): boolean {
    ...
    if (field.getType() === FieldTypesFormatter.MESSAGE_TYPE) {
        return true;
    }
...
export function format(fileName: string,
    ...
    descriptor.getFieldList().forEach((field: FieldDescriptorProto) => {
        ...
        if (fieldData.type === FieldTypesFormatter.MESSAGE_TYPE) {
            ...
            if (fieldData.isMapField) {
                ...
                if (valueType === FieldTypesFormatter.BYTES_TYPE) {
        ...
        } else if (fieldData.type === FieldTypesFormatter.ENUM_TYPE) {
        ...
        fieldData.isOptionalValue = field.getType() === FieldTypesFormatter.MESSAGE_TYPE;
        fieldData.isRepeatField = field.getLabel() === FieldDescriptorProto.Label.LABEL_REPEATED;
        if (!fieldData.isRepeatField && fieldData.type !== FieldTypesFormatter.BYTES_TYPE) {
    ...
    return TplEngine.render('partial/message', {
        ...
        BYTES_TYPE: FieldTypesFormatter.BYTES_TYPE,
        MESSAGE_TYPE: FieldTypesFormatter.MESSAGE_TYPE,
        message: messageData,
    });
}
